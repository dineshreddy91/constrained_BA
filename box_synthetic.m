...
for num2=1:size(num_all,2)
%load('bundle_data_moving_7.mat');
num_of_rand_points=num_all(num2);
...
scatter3(pointCloud(1,1:size(pointCloud,1)),pointCloud(2,1:size(pointCloud,1)),pointCloud(3,1:size(pointCloud,1)));
...
error_1=[sigma num_of_rand_points error];
dlmwrite('myFile.txt',error_1,'-append');
error_all(num)=error;
end
end
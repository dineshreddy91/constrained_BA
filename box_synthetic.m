...
for c_iter = 1:num_constraints
	for m_iter = 1:length(mode)
		...
		load( 'bundle_data_moving.mat' ); 															% Make sure all 3D parameters are reset every iteration.
		...
		figure(1); scatter3( pointCloud(1, :), pointCloud(2, :), ...
							 pointCloud(3, :) ); 
							 title( 'Before Adjustment' );
	%	axis( [-5 5 -6 6 0 30] );
		
		[ cameraRtC2W1, point_cloud ]	=	bundleAdjustment2D3DBoxFile( ...
												cameraRtC2W, pointCloud, pointObserved, ...
												pointObservedValue, K, w3D, mode(m_iter), num_of_rand_points);
		
		% Plot 3D points after doing bundle adjustment.
		figure(2); scatter3( point_cloud(1,:), point_cloud(2, :), ...
							 point_cloud(3, :) ); title( 'After Adjustment' );
		%axis( [-5 5 -6 6 0 30] );
